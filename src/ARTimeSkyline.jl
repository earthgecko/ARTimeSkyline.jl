# This code is derived from https://github.com/markNZed/ARTimeNAB.jl/blob/main/src/ARTime.jl
# released under the AGPL3 license.  It also uses the "compacted" version of
# AdaptiveResonance.jl from https://github.com/markNZed/ARTimeNAB.jl
# which is based on the excellent AdaptiveResonance project (licensed under MIT).
#
# The changes from ARtime.jl here allow for all parameters to be configurable so
# that they are not hardcoded can be passed to allow for testing and
# experimenting with different parameters and validating parameters are valid
# where required to ensure no errors are encountered due to uneven parameter
# values being passed.
module ARTimeSkyline
...
@kwredef mutable struct P
    ...
    # @added 20220520 - Task #4582 - POC ARTime
    learn_all::Bool = false
    spike::Float64 = 0.1
end

# @modified 20220520 - Task #4582 - POC ARTime
# Allow the parameters to be passed to the function rather than being declared
# statically
# function init(dmin, dmax, dlength, p=p)
function init(
    dmin, dmax, dlength, window::Int=8, probationary_period::Int=750,
    windows_per_pb::Int=13, sstep::Int=1, discretize_chomp::Float64=0.075,
    nlevels::Int=80, mask_rho_after_anomaly::Int=80, trend_window::Int=100,
    initial_rho::Float64=0.80, learn_all::Bool=false, spike::Float64=0.1, p=p)
    p.dmin = dmin
    p.dmax = dmax
    p.dlength = dlength
    # @modified 20220520 - Task #4582 - POC ARTime
    # This probationary_period is derived from NAB criteria which requires no
    # scoring to be done on the first 750 data points of the data set or the
    # first 15% if the data set is less that 5000 data points long.  In general
    # terms this resulted in a p.sstep value of 107.  Changing or discarding the
    # probationary_period has a significant effect on the outcome of the
    # analysis because drastically changes the sstep and trend_window.
    # Further ensure that the probationary_period is an even number to ensure
    # when a very short probationary_period is used it does not misalign with
    # the window and result in an error.
    # probationary_period = dlength < 5000 ? Int.(floor(0.15 * dlength)) : 750
    p.probationary_period = probationary_period - mod(probationary_period, 2)
    if p.probationary_period > 750 p.probationary_period = 750 end
    # @modified 20220520 - Task #4582 - POC ARTime
    # Ensure the window is even as well otherwise errors are encountered like
    # JuliaError: InexactError: Int64(22.5)
    p.window = window - mod(window, 2)
    # @modified 20220520 - Task #4582 - POC ARTime
    # The probationary_period must => the p.window otherwise anomaly scores are nans
    if p.probationary_period < p.window * 2 p.probationary_period = p.window * 2 end
    p.windows_per_pb = windows_per_pb
    p.trend_window = trend_window
    p.discretize_chomp = discretize_chomp
    p.nlevels = nlevels
    p.mask_rho_after_anomaly = mask_rho_after_anomaly
    p.trend_window = trend_window
    p.initial_rho = initial_rho
    p.spike = spike
...
function process_sample!(di, p=p)
    ...
    if mod(i,p.sstep) == 0
        ...
        # @modified 20220520 - Task #4582 - POC ARTime
        # Allow spike variable to be passed in for testing performance
        # if abs(ds - mean) < (0.1 * mean) # spike must be at least 10%
        if abs(ds - mean) < (p.spike * mean) # spike must be at least p.spike %
            ds = mean
        end
        # Normalize
        ds = ds - p.dmin
...
function detect!(f, i, p)
    ...
    if i < p.window
        # @modified 20220520 - Task #4582 - POC ARTime
        # See what try learning all does if 7 day data represents a sustained
        # spike for an hour on a Friday and the following Friday if that spike
        # occurs and it was not learnt then it well be flagged as anomalous.  As
        # long as it falls in the initial probationary_period even with learning
        # set to true it will still fire as it is not learnt because it falls in
        # the probationary_period.
        # Further if learning is set to true the results are very different and
        # so is the runtime.
        # AdaptiveResonance.train!(p.cs.art, f, learning=false)
        AdaptiveResonance.train!(p.cs.art, f, learning=p.learn_all)
        cat = -1
    else
